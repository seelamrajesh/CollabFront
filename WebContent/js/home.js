var ChatBack=angular.module("ChatBack",['ngRoute','ngCookies']);
ChatBack.config(function($routeProvider){
	$routeProvider
...
ChatBack.directive('fileModel', ['$parse', function ($parse) {
    return {
       restrict: 'A',
...
ChatBack.service('fileUpload', ['$http','$location', function ($http,$scope,$location) {
...
ChatBack.run( function ($rootScope, $location,$cookieStore, $http) {
...
ChatBack.controller('registerController', ['$scope', 'fileUpload', function($scope,fileUpload){
    $scope.register = function(){
       ...
       console.log('file is ' );
       console.dir(file);
       var uploadUrl = "http://localhost:2020/ChatBack/fileUpload";
...
ChatBack.service("ChatService", function($q, $timeout) {
    ...
    service.SOCKET_URL = "/ChatBack/chat";
...
ChatBack.controller("chatController",function($scope,$http,ChatService,$rootScope)
		{
	...
	$scope.messages = [];
	  ...
	  ChatService.receive().then(null, null, function(message) {
		...
		ChatBack.controller("JobsController",function($scope,$http)
				{
			...
			$scope.addJ=function()
			{
				...
				var res=$http.post("http://localhost:2020/ChatBack/addJobs",jobs);
				...
				ChatBack.controller("loginController",['$cookieStore','$scope','$http','$location','$rootScope',function($cookieStore,$scope,$http,$location,$rootScope)
						{
					...
					$http.post("http://localhost:2020/ChatBack/authenticate",logi).then(function(response)
							{
				...
				ChatBack.controller("blogController",function($scope,$http,$rootScope)	
						{
					$rootScope.userhome=true;
					$rootScope.userforum=true;
					$rootScope.userjobs=true;
					...
					$http.get("http://localhost:2020/ChatBack/viewMyBlogs/"+$rootScope.uname)
							 ...
							 $scope.newBlog={};
								...
								$scope.addBlog=function(newBlog)
								{
									...
									console.log("title:"+dataObj);
									 var res = $http.post('http://localhost:2020/ChatBack/addBlog',dataObj);
									 $http.get("http://localhost:2020/ChatBack/viewMyBlogs/"+$rootScope.uname)
				...
				$scope.saveEdit=function()
				{
					...
					$http.put("http://localhost:2020/ChatBack/updateBlog",edit);
					$http.get("http://localhost:2020/ChatBack/viewMyBlogs/"+$rootScope.uname)
					    .then(function (response) {
				...
				$http.post("http://localhost:2020/ChatBack/deleteBlog",del);
					$http.get("http://localhost:2020/ChatBack/viewMyBlogs/"+$rootScope.uname)
					    .then(function (response) {
				...
				ChatBack.controller("adminBlogController",function($scope,$http,$rootScope)	
						{	
					$rootScope.home=false;
					$rootScope.login=false;
					$rootScope.register=false;
					...
					console.log(" in adminblog controller");
							 ...
							 $http.get("http://localhost:2020/ChatBack/viewBlogs")
							    .then(function (response) {
				...
				ChatBack.controller("adminJobsController",function($scope,$http,$rootScope)	
						{	
					$rootScope.home=false;
					$rootScope.login=false;
					$rootScope.register=false;
					...
					console.log(" in jobs controller");
							 ...
							 $http.get("http://localhost:2020/ChatBack/viewJobs")
							    .then(function (response) {
							 ...
							 $scope.newJobs={};
								...
								$scope.addJobs=function(newJobs)
								{
									...
									console.log("title:"+dataObj);
									 var res = $http.post('http://localhost:2020/ChatBack/addJobs',dataObj);
									 $http.get("http://localhost:2020/ChatBack/viewJobs")
				...
				$scope.saveEdit=function()
				{
					...
					$http.put("http://localhost:2020/ChatBack/updateJob",edit);
					 $http.get("http://localhost:2020/ChatBack/viewJobs")
					    .then(function (response) {
				...
				$http.post("http://localhost:2020/ChatBack/deleteJob",del);
					 $http.get("http://localhost:2020/ChatBack/viewJobs")
					    .then(function (response) {
				...
				ChatBack.controller('logoutController',function($scope,$rootScope,$http,$cookieStore)		
						{
					$http.post("http://localhost:2020/ChatBack/logout/"+$rootScope.uname);
				...
				ChatBack.controller("userJobsController",function($scope,$http,$rootScope)	
						{	
					$rootScope.userhome=true;
					$rootScope.userforum=true;
					$rootScope.userjobs=true;
				...
				ChatBack.controller("adminForumController",function($scope,$http,$rootScope)	
						{	
					$rootScope.home=false;
					$rootScope.login=false;
					$rootScope.register=false;
					...
					console.log(" in adminforum controller");
							 ...
							 $http.get("http://localhost:2020/ChatBack/viewForums")
							    .then(function (response) {
							 ...
							 $scope.newForum={};
								...
								$scope.addForum=function(newForum)
								{
									...
									console.log("title:"+dataObj);
									 var res = $http.post('http://localhost:2020/ChatBack/addForum',dataObj);
									 $http.get("http://localhost:2020/ChatBack/viewForums")
				...
				$scope.approveForum=function()
				{
					...
					$http.put("http://localhost:2020/ChatBack/updateForum",edit);
					 $http.get("http://localhost:2020/ChatBack/viewForums")
					    .then(function (response) {
				...
				ChatBack.controller("userForumController",function($scope,$http,$rootScope)	
						{	
					$rootScope.userhome=true;
					$rootScope.userforum=true;
					$rootScope.userjobs=true;
					...
					console.log(" in forum controller");
							 ...
							 $http.get("http://localhost:2020/ChatBack/viewForum")
							    .then(function (response) {
							 ...
							 $scope.newForum={};
								...
								$scope.answer=function(forum)
								{
									...
									answer:$scope.answerforum.answer
									 ...
									 $http.post('http://localhost:2020/ChatBack/answerForum',ans);
									$http.get("http://localhost:2020/ChatBack/viewForum")
								    .then(function (response) {
								...
								$scope.viewanswers=function(forum)
								{
									...
									console.log("forumid:"+$scope.answerforum.forum_id);
									 $http.get("http://localhost:2020/ChatBack/viewAnswers/"+$scope.answerforum.forum_id)
									    .then(function (response) {
				...
				$http.post("http://localhost:2020/ChatBack/deleteForum",del)
				...
				ChatBack.controller("userHomeController",function($scope,$http,$rootScope)	
						{	
					...
					console.log("name in  findfriends:"+$rootScope.uname);
							 $http.get("http://localhost:2020/ChatBack/findFriends/"+$rootScope.uname)
							    .then(function (response) {
					...
					$scope.addfriend=function(user)
					{
						...
						$http.post("http://localhost:2020/ChatBack/addFriend/",fr);
						
					}
					
						});
				ChatBack.controller("friendslistController",function($scope,$http,$rootScope)	
						{	
					$rootScope.userhome=true;
					$rootScope.userforum=true;
					$rootScope.userjobs=true;
					...
					console.log("name in  friendslist:"+$rootScope.uname);
							 $http.get("http://localhost:2020/ChatBack/viewFriends/"+$rootScope.uname)
							    .then(function (response) {
				...
				ChatBack.controller("allblogsController",function($scope,$http,$rootScope,$route)	
						{	
					$rootScope.userhome=true;
					$rootScope.userforum=true;
					$rootScope.userjobs=true;
					...
					console.log("username in allblog controller:"+$rootScope.uname);
							 $http.get("http://localhost:2020/ChatBack/viewAllBlogs")
							    .then(function (response) {
							 ...
							 { 
						       ...
						       var like=
								...
								console.log("postedby:"+$rootScope.uname);
								 $http.put('http://localhost:2020/ChatBack/updateBlog',like);
								 $http.get("http://localhost:2020/ChatBack/viewBlogs")
								    .then(function (response) {
					 ...
					 $http.get("http://localhost:2020/ChatBack/viewComments/"+$scope.viewcomments.blog_id)
								    .then(function (response) {
						...
						$scope.comment=function(allblogs)
						{
							...
							$http.post('http://localhost:2020/ChatBack/addComment',comment);
							 $http.get("http://localhost:2020/ChatBack/viewBlogs")
							    .then(function (response) {
